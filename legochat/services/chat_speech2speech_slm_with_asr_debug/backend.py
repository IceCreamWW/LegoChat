...
class ChatSpeech2SpeechSession:
    ...
    def __init__(
        ...
        self.transcribe = (
            ...
            if getattr(self.speech2text, "is_streaming", False)
            else self.transcribe_non_streaming
        )
    ...
    async def detect_speech(self):
        ...
        for result in vad_results:
            ...
            if "end" in result:
                self.user_is_speaking = False
                self.voiced_segments[-1]["end"] = max(
    ...
    async def transcribe_non_streaming(self, end_of_stream=False):
        ...
        text, self.speech2text_states = await asyncio.to_thread(
            self.speech2text.process,
            samples=self.speech,
            states=self.speech2text_states,
